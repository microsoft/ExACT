...
async def aprocess_accessibility_tree_node(
    ...
    if "backendDOMNodeId" not in node:
        ...
        if node["role"]["value"] == "RootWebArea":
            ...
            response = await aget_bounding_client_rect(
                client,
                backend_node_id
            )
...
async def process_accessibility_tree(
    ...
    await atqdm.gather(
        *tasks,
        total=len(tasks),
        desc="Aprocessing accessibility tree nodes"
    )
    return


class FastTextObervationProcessor(ObservationProcessor):
    def __init__(
        self,
    ...
    async def afetch_page_accessibility_tree(
        ...
        accessibility_tree: AccessibilityTree = _resp["nodes"]
        ...
        ### asyncio code
        nodeid_to_cursor = {}
        await process_accessibility_tree(
            accessibility_tree,
            client,
            nodeid_to_cursor
        )
    ...
    @atime_it
    async def afetch_image_related(self, page: Page, browser_info: BrowserInfo) -> str:
        ...
        if page.url.endswith((".jpg", ".jpeg", ".png")):
            ...
            if self.captioning_fn is not None:
                ...
                for image in images:
                    try:
                        original_alt = await image.get_attribute("alt") or ""
                        image_url = await image.get_attribute("src")
                        if not image_url.startswith(("http://", "https://", "www.")):
                            image_url = urljoin(page.url, image_url)
                        ...
                        await image.evaluate(f"node => node.alt = {safe_updated_alt}")
                    except Exception as e:
                        print("L653 WARNING:", e)

            if self.observation_type == "accessibility_tree_with_captioner":
                frame_ax_trees = await self.afetch_page_accessibility_tree(
                    page,
                    browser_info,
    ...
    @atime_it
    async def aprocess(self, page: Page) -> str:
        # get the tab info
        ...
        if self.observation_type == "html":
            ...
            content = await self.afetch_image_related(
                page,
                browser_info,
            )
    ...
    def process(self, page: Page) -> str:
        # return asyncio.run(self.aprocess(page))
        raise NotImplementedError  # only aprocess should be used
...
class FastCachedTextObervationProcessor(FastTextObervationProcessor):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.obs_cache = {}
        self.metadata_cache = {}
        self.browser_config_cache = {}
    ...
    @atime_it
    async def cached_aprocess(self, page: Page, cache_id: str) -> str:
        self._cache_hit_stats['num_visit'] += 1
        if cache_id in self.obs_cache:
            content = self.obs_cache[cache_id]
            metadata = self.metadata_cache[cache_id]
            browser_config = self.browser_config_cache[cache_id]
            self._cache_hit_stats['num_hit'] += 1
            logger.debug(f"text obs hit cache with {cache_id=}")

            ## sync metadata
            ## fields that will be set in aprocess needs to be manually set here
            self.meta_data = copy.deepcopy(metadata)
            if 'obs_nodes_info' in self.meta_data:
                self.obs_nodes_info = self.meta_data['obs_nodes_info']
            self.browser_config = copy.deepcopy(browser_config)
        else:
            content = await self.aprocess(page)
            metadata = self.meta_data
            browser_config = self.browser_config
            logger.debug(f"text obs adding cache with {cache_id=}, currently {self.obs_cache.keys()=}")

            self.obs_cache[cache_id] = content
            self.metadata_cache[cache_id] = copy.deepcopy(metadata)
            self.browser_config_cache[cache_id] = copy.deepcopy(browser_config)
        logger.debug(
            f'text obs cache hit rate: {self._cache_hit_stats["num_hit"]}/{self._cache_hit_stats["num_visit"]}'
        )
        return content


class FastImageObservationProcessor(ObservationProcessor):
    def __init__(
        self,
        observation_type: str,
    ...
    async def aget_page_bboxes(self, page: Page) -> list[list[float]]:
        ...
        csv_content = await page.evaluate(js_script)
        return csv_content
    ...
    def draw_bounding_boxes(
        ...
        font_path = "data/visualwebarena/media/SourceCodePro-SemiBold.ttf"
    ...
    @atime_it
    async def aprocess(self, page: Page) -> tuple[npt.NDArray[np.uint8], npt.NDArray[np.uint8], str]:
        try:
            browser_info = await self.afetch_browser_info(page)
        except Exception:
            await page.wait_for_load_state("load", timeout=500)
        ...
        if self.observation_type == "image_som":
            ...
            try:
                ...
                som_bboxes = await self.aget_page_bboxes(page)
    ...
    @atime_it
    async def afetch_browser_info(self, page: Page) -> BrowserInfo:
        client = await page.context.new_cdp_session(page)
        # extract domtree
        tree = await client.send(
            "DOMSnapshot.captureSnapshot",
...
class FastCachedImageObservationProcessor(FastImageObservationProcessor):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.obs_cache = {}
        self.metadata_cache = {}
    ...
    @atime_it
    async def cached_aprocess(self, page: Page, cache_id: str) -> tuple[npt.NDArray[np.uint8], str]:
        self._cache_hit_stats['num_visit'] += 1
        if cache_id in self.obs_cache:
            image_obs, screenshot, content_str = self.obs_cache[cache_id]
            metadata = self.metadata_cache[cache_id]
            browser_config = self.browser_config_cache[cache_id]
            self._cache_hit_stats['num_hit'] += 1
            logger.debug(f"image obs hit cache with {cache_id}")

            ## sync metadata
            ## fields that will be set in aprocess needs to be manually set here
            self.meta_data = copy.deepcopy(metadata)
            if 'obs_nodes_info' in self.meta_data:
                self.som_id_info = self.meta_data['obs_nodes_info']
            self.browser_config = copy.deepcopy(browser_config)
        else:
            image_obs, screenshot, content_str = await self.aprocess(page)
            metadata: ObservationMetadata = self.meta_data
            browser_config = self.browser_config
            logger.debug(f"image obs adding cache with {cache_id=}, currently {self.obs_cache.keys()=}")

            self.obs_cache[cache_id] = (copy.deepcopy(image_obs), copy.deepcopy(screenshot), content_str)
            self.metadata_cache[cache_id] = copy.deepcopy(metadata)
            self.browser_config_cache[cache_id] = copy.deepcopy(browser_config)
        logger.debug(
            f'image obs cache hit rate: {self._cache_hit_stats["num_hit"]}/{self._cache_hit_stats["num_visit"]}'
        )
        return image_obs, screenshot, content_str


class FastObservationHandler:
    ...
    def __init__(
        ...
        self.text_processor = FastTextObervationProcessor(
            text_observation_type,
            current_viewport_only,
            viewport_size,
            captioning_fn,
        )
        self.image_processor = FastImageObservationProcessor(
    ...
    async def aget_observation(self, page: Page) -> dict[str, Observation]:
        text_obs = await self.text_processor.aprocess(page)
        image_obs, content_str = await self.image_processor.aprocess(page)
        if content_str != "":
...
class FastCachedObservationHandler(FastObservationHandler):
    def __init__(
        self,
        main_observation_type: str,
        text_observation_type: str,
        ...
        self.main_observation_type = main_observation_type
        self.text_processor = FastCachedTextObervationProcessor(
            text_observation_type,
            current_viewport_only,
            viewport_size,
            captioning_fn,
        )
        self.image_processor = FastCachedImageObservationProcessor(
            image_observation_type, viewport_size
        )
        self.viewport_size = viewport_size
        return
    ...
    async def cached_aget_observation(self, page: Page, cache_id: str) -> dict[str, Observation]:
        text_obs = await self.text_processor.cached_aprocess(page, cache_id)
        image_obs, raw_screenshot, content_str = await self.image_processor.cached_aprocess(page, cache_id)
        if content_str != "":
            text_obs = content_str
        return {"text": text_obs, "image": image_obs, "raw_screenshot": raw_screenshot}