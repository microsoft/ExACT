...
from browser_env.actions import Action as OldAction
from browser_env import ActionParsingError
from src.envs.processors import ObservationProcessor
...
class Action:
    ...
    def to_simple_str(self):
        ...
        for field in fields(self):
            if field.name == 'metadata':
                continue
            data += f'{field.name}={getattr(self, field.name)}\n'

        ## format some metadata
        for k, v in self.metadata.items():
            if 'obs_metadata' == k:
                obs_metadata = v
                if 'text' in obs_metadata:
                    obs_nodes_info = obs_metadata['text'].get('obs_nodes_info', {})
                    data += f'(text) obs_nodes_info={obs_nodes_info.keys()}\n'
                if 'image' in obs_metadata:
                    obs_nodes_info = obs_metadata["image"].get("obs_nodes_semantic_info", {})
                    data += f'(image) obs_nodes_semantic_info={obs_nodes_info.keys()}\n'
            if 'previous_' in k:
                data += f'(others) {k}={v}\n'
        return data.strip()
    ...
    @staticmethod
    def from_old_action(action: OldAction) -> "Action":
        new_action = create_none_action()
        new_action.update(action)
        return new_action

    def __hash__(self):
        # str of everything except metadata
        data = ''
        for field in fields(self):
...
@beartype
def actionhistory2str(
    action_history: list[Action], action_set_tag: str
) -> str:
    """Return the string representation of an action history"""
    action_strs = []
