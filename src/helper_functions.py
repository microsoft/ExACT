...
from browser_env import ObservationMetadata, StateInfo
        ...
        case "id_accessibility_tree":
            ...
            if action["element_id"] in text_meta_data["obs_nodes_info"]:
                ...
                if action["element_id"] != "":
                    logger.debug(f"render: [{action['element_id']}] is NOT found on the page!")
                    logger.debug(f"render: raw_prediction: {action['raw_prediction']}")
                    logger.debug(f"render: obs_nodes_info:\n{text_meta_data['obs_nodes_info'].keys()}")
        ...
        case "som":
            ...
            if action["element_id"] in meta_data["obs_nodes_semantic_info"]:
                ...
                if action["element_id"] != "":
                    logger.debug(f"render: [{action['element_id']}] is NOT found on the page!")
                    logger.debug(f"render: raw_prediction: {action['raw_prediction']}")
                    logger.debug(f"render: obs_nodes_info:\n{meta_data['obs_nodes_semantic_info'].keys()}")
                node_content = "No match found"
...
class RenderHelper(object):
    ...
    def render(
        ...
        # action metadata contains too much info. do not render it here
